...
import type { ChannelProperty, CharacterRigConfig, EditorProject } from "./types.js";

const VALID_EDITOR_CHANNEL_PROPERTIES: ChannelProperty[] = ["tx", "ty", "tz", "rot", "tilt", "spin"];

export type ValidationIssue = { path: string; message: string };

function validateCharacterRigConfig(
  rig: CharacterRigConfig,
  pathPrefix: string,
  ids: Set<string>,
  issues: ValidationIssue[],
  globalSliceIds: Set<string>,
): void {
  const sheets = rig.spriteSheets ?? [];
  const sheetIds = new Set<string>();
  for (const sh of sheets) {
    if (sheetIds.has(sh.id)) {
      issues.push({ path: `${pathPrefix}.spriteSheets`, message: `duplicate sprite sheet id: ${sh.id}` });
    }
    sheetIds.add(sh.id);
    if (typeof sh.fileName !== "string" || sh.fileName.length === 0) {
      issues.push({ path: `${pathPrefix}.spriteSheets.${sh.id}.fileName`, message: "sprite sheet needs a file name" });
    }
    if (typeof sh.dataBase64 !== "string" || sh.dataBase64.length === 0) {
      issues.push({ path: `${pathPrefix}.spriteSheets.${sh.id}.dataBase64`, message: "sprite sheet payload is empty" });
    }
    const norm = normalizeReferenceImageMime(sh.mimeType);
    if (!norm) {
      issues.push({
        path: `${pathPrefix}.spriteSheets.${sh.id}.mimeType`,
        message: "unsupported sprite sheet type (use PNG, JPEG, or WebP)",
      });
    }
  }
  for (const s of rig.slices) {
    if (globalSliceIds.has(s.id)) {
      issues.push({ path: `${pathPrefix}.slices`, message: `duplicate slice id: ${s.id}` });
    }
    globalSliceIds.add(s.id);
    if (typeof s.name !== "string" || s.name.length === 0) {
      issues.push({ path: `${pathPrefix}.slices.${s.id}.name`, message: "slice needs a name" });
    }
    const emptySlot = s.width <= 0 || s.height <= 0;
    if (!emptySlot && (!(s.width > 0) || !(s.height > 0))) {
      issues.push({ path: `${pathPrefix}.slices.${s.id}`, message: "slice width/height must be positive when not empty" });
    }
    if (!Number.isFinite(s.worldCx) || !Number.isFinite(s.worldCy)) {
      issues.push({ path: `${pathPrefix}.slices.${s.id}`, message: "slice world position must be finite" });
    }
    if (s.side != null && s.side !== "front" && s.side !== "back") {
      issues.push({ path: `${pathPrefix}.slices.${s.id}.side`, message: "side must be front or back" });
    }
    if (emptySlot) {
      continue;
    }
    if (s.embedded && s.sheetId) {
      issues.push({ path: `${pathPrefix}.slices.${s.id}`, message: "slice cannot have both embedded and sheetId" });
    }
    if (!s.embedded && !s.sheetId) {
      issues.push({
        path: `${pathPrefix}.slices.${s.id}`,
    ...
    if (s.embedded) {
      ...
      if (!norm) {
        issues.push({
          path: `${pathPrefix}.slices.${s.id}.embedded`,
          message: "unsupported embedded image type (use PNG, JPEG, or WebP)",
        });
      }
      if (typeof em.dataBase64 !== "string" || em.dataBase64.length === 0) {
        issues.push({ path: `${pathPrefix}.slices.${s.id}.embedded`, message: "embedded image payload is empty" });
      }
      if (!(em.pixelWidth > 0) || !(em.pixelHeight > 0)) {
        issues.push({ path: `${pathPrefix}.slices.${s.id}.embedded`, message: "embedded dimensions invalid" });
      }
    ...
    if (s.embeddedBack) {
      ...
      if (!norm) {
        issues.push({
          path: `${pathPrefix}.slices.${s.id}.embeddedBack`,
    ...
    if (s.sheetId && !s.embedded) {
      if (!sheetIds.has(s.sheetId)) {
        issues.push({ path: `${pathPrefix}.slices.${s.id}.sheetId`, message: "unknown sprite sheet id" });
      } else {
  ...
  const sliceIds = new Set(rig.slices.map((s) => s.id));
  const boundSlice = new Set<string>();
  for (const b of rig.bindings ?? []) {
    if (!sliceIds.has(b.sliceId)) {
      issues.push({ path: `${pathPrefix}.bindings`, message: `unknown slice id in binding: ${b.sliceId}` });
    }
    if (!ids.has(b.boneId)) {
      issues.push({ path: `${pathPrefix}.bindings`, message: `unknown bone id in binding: ${b.boneId}` });
    }
    if (boundSlice.has(b.sliceId)) {
      issues.push({ path: `${pathPrefix}.bindings`, message: `duplicate binding for slice: ${b.sliceId}` });
    }
  ...
  for (const d of rig.sliceDepths ?? []) {
    if (!sliceIds.has(d.sliceId)) {
      issues.push({ path: `${pathPrefix}.sliceDepths`, message: `unknown slice id in depth: ${d.sliceId}` });
    }
    if (depthSlice.has(d.sliceId)) {
      issues.push({ path: `${pathPrefix}.sliceDepths`, message: `duplicate depth entry for slice: ${d.sliceId}` });
    }
    ...
    const checkTex = (label: "Front" | "Back", tex?: { mimeType: string; dataBase64: string; pixelWidth: number; pixelHeight: number }) => {
      ...
      if (!norm) {
        issues.push({
          path: `${pathPrefix}.sliceDepths.${d.sliceId}.depthTexture${label}`,
      ...
      if (s && (tex.pixelWidth !== s.width || tex.pixelHeight !== s.height)) {
        issues.push({
          path: `${pathPrefix}.sliceDepths.${d.sliceId}`,
          message: "depth texture size must match slice width/height",
        });
      }
    };
    checkTex("Front", (d as { depthTextureFront?: { mimeType: string; dataBase64: string; pixelWidth: number; pixelHeight: number } }).depthTextureFront);
    checkTex("Back", (d as { depthTextureBack?: { mimeType: string; dataBase64: string; pixelWidth: number; pixelHeight: number } }).depthTextureBack);
  }
}
...
export function validateEditorProject(project: EditorProject): ValidationIssue[] {
  ...
  const globalSliceIds = new Set<string>();

  if (project.characters?.length) {
    const charIds = new Set<string>();
    for (const c of project.characters) {
      if (charIds.has(c.id)) {
        issues.push({ path: "characters", message: `duplicate character id: ${c.id}` });
      }
      charIds.add(c.id);
      if (typeof c.name !== "string" || c.name.length === 0) {
        issues.push({ path: `characters.${c.id}.name`, message: "character needs a name" });
      }
      if (!ids.has(c.rootBoneId)) {
        issues.push({ path: `characters.${c.id}.rootBoneId`, message: `unknown bone: ${c.rootBoneId}` });
      }
    }
    if (project.characterRigs) {
      for (const slotId of Object.keys(project.characterRigs)) {
        if (!charIds.has(slotId)) {
          issues.push({ path: `characterRigs.${slotId}`, message: "orphan rig (no matching character slot)" });
        }
      }
      for (const [slotId, rig] of Object.entries(project.characterRigs)) {
        validateCharacterRigConfig(rig, `characterRigs.${slotId}`, ids, issues, globalSliceIds);
      }
    }
    for (const c of project.characters) {
      if (project.characterRigs && !project.characterRigs[c.id]) {
        issues.push({ path: `characters.${c.id}`, message: "missing characterRigs entry for slot" });
      }
    }
  }

  if (project.characterRig) {
    validateCharacterRigConfig(project.characterRig, "characterRig", ids, issues, globalSliceIds);
  }
